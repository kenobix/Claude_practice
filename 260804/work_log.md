...
**読み取れる結果**: zidane.jpgから2人(score 0.997/0.993)、bus.jpgから4人(score 0.975〜0.999)を検出し、いずれも17点全てのキーポイントが可視と判定された。骨格線を重ねると、しゃがんだ姿勢や腕を組んだ姿勢など人物ごとに異なるポーズが視覚的にも正しく再現されており、Mask R-CNNと同じ「ROIごとにブランチを追加する」トップダウン方式で、複数人物・多様な姿勢でも個別に関節位置を推定できることを確認した。G検定で対比されるOpenPose(ボトムアップ方式: 先に画像全体のキーポイント候補とPart Affinity Fieldsを求めてから人物に割り当てる)とは逆のアプローチだが、出力される情報(人物ごとの関節座標の集合)自体は共通しており、「検出→その中身を詳しく見る」というFaster R-CNN系の基本設計がクラス分類・画素マスク・関節座標のいずれにも応用できる汎用性を持つことが分かった。
...
**読み取れる結果**: 最終L1誤差(画素値の平均絶対誤差)=0.1757。生成結果を見ると、入力エッジ画像の輪郭線の位置・形状(円/四角/三角/十字)はほぼそのまま保たれており、スキップ結合によって入力の構造情報がデコーダまで直接伝わっていることが確認できた。一方で塗られた色は正解画像の色と必ずしも一致しない——これは学習の失敗ではなく、エッジ画像自体に色の情報が全く含まれていないため「この形に元々どんな色が塗られていたか」はモデルにとって本質的に決定不能な問題であり、学習データ全体でもっともらしい色調(この合成図形データセットでは暗めの色)を推測して塗っていると考えられる。敵対損失の推移はGeneratorとDiscriminatorが互いに反応し合いながら拮抗する典型的なGANの挙動を示し、L1誤差は単調に減少した。02のDCGAN(ノイズ→画像の1対多写像)と対比すると、Pix2Pixは「入力画像→対応する1枚」というほぼ1対1の写像を学習する設計だが、この色の例のように入力だけでは一意に定まらない要素は依然として残ることが分かり、同じGANの枠組みでもタスク設計によって生成の自由度・決定性が大きく変わることを確認できた。
...
### 図5: double_dqn_dueling.png — Double DQNとDueling Network（[05_double_dqn_dueling.py](stage12/05_double_dqn_dueling.py)）

![vanilla DQN/Double DQN/Dueling Networkの学習曲線と、vanilla DQN/Double DQNのQ値過大評価バイアス比較](stage12/double_dqn_dueling.png)

**何を示す図か**: vanilla DQNの「次状態の行動選択と評価を同じネットワークで行う」ことによるQ値の過大評価バイアスを、Double DQN(選択はオンラインネット、評価はターゲットネットで分離)がどれだけ緩和するかを、初期状態でのQ値推定と貪欲方策による実際の収益との差として直接測定した。あわせてDueling Network(V(s)とA(s,a)を分離して合成する構造)も学習曲線で比較した。

**読み取れる結果**: Q値のバイアス(Q推定−実際の収益)はvanilla DQN=−43.84、Double DQN=−59.89となり、理論が想定する「Q推定が実際の収益を上回る(正の過大評価)」とは逆に、どちらも負のバイアス(過小評価)になった。これは評価に使った「実際の収益」が探索なしの貪欲方策によるロールアウトである一方、Q関数はε-greedyで探索も混ざった250エピソード分のデータからしか学習しておらず、学習後半で急速に上達した貪欲方策の実力にQ関数の推定(ブートストラップ)が追いついていないことが原因と考えられる——Double DQNの過大評価抑制効果を確認するには、方策が収束しきった後で評価するか、より長い学習が必要だったことを示す結果になった。学習曲線ではDouble DQNが最後の50エピソード平均230.3で最も高く、Dueling Networkは194.4とやや劣ったが、CartPoleの行動数が2(左/右)と少ないため「行動によらない状態価値と行動ごとの優位性を分離する」というDueling構造の恩恵が出にくいタスクだったと考えられる。

### 図6: reward_shaping.png — 報酬形成(Reward Shaping)（[06_reward_shaping.py](stage12/06_reward_shaping.py)）

![MountainCarでの形成なし/naive(位置ベース)/energy(エネルギーベース)のゴール到達率・ステップ数の推移](stage12/reward_shaping.png)

**何を示す図か**: 疎な報酬(-1/ステップ、ゴールでのみ終了)を持つMountainCarで、tabular Q学習に (1) 報酬形成なし (2) 位置だけを評価するnaiveなポテンシャルベース形成 (3) 位置エネルギー+運動エネルギー(速度の2乗)を評価するenergyベースの形成、の3種類を適用し3000エピソード学習して比較した。

**読み取れる結果**: 直近500エピソードの成功率は、形成なし=48.2%、naive(位置)=58.2%、energy(エネルギー)=61.4%となり、energyが最も高い成功率を達成した。MountainCarの最適方策は「一度ゴールと逆方向に後退して勢いをつけてから加速する」ことを含むため、位置(ゴールへの近さ)だけを評価するnaiveな形成は後退行動を評価しにくくする可能性がある一方、エネルギー(方向によらず加速そのもの)を評価するenergy形成はこのタスク構造に適合していたと考えられる。ポテンシャルベースの報酬形成は理論上「最適方策自体は変えない」ことが保証されているが、どのポテンシャル関数を選ぶかで学習の「速さ」は大きく変わりうるという、理論的な安全性と実践上の設計難易度が別問題であることを実測で確認できた。

### 図7: actor_critic.png — Actor-Critic（[07_actor_critic.py](stage12/07_actor_critic.py)）

![Actor-Critic(1ステップ更新)とREINFORCE(エピソード単位更新)のCartPole学習曲線比較](stage12/actor_critic.png)

**読み取れる結果**: REINFORCEは529エピソードで解けた基準(直近100エピソード平均475以上)に到達した一方、Actor-Criticは上限800エピソードまでに基準に届かず、最終100エピソード平均はわずか9.3にとどまった。しかし学習曲線を詳しく見ると、Actor-Criticの移動平均は一度エピソード430付近で240程度まで着実に上昇しており、単に学習が進まなかったのではなく、学習の途中で方策が突然崩壊し、その後800エピソードまで一切回復しないまま終わっていることが分かった。これは「破局的忘却」と呼ばれる現象で、1ステップごとにActorとCriticを同時に更新するオンラインActor-Criticでは、たまたま悪い方向に更新が振れると方策(softmax)が特定の行動に急速に偏り(確率が0や1に近づき)、その状態からは勾配がほぼ消えるため方策を立て直す手立てを失う、という弱点が実際に再現されたと考えられる。REINFORCEはエピソード全体の実収益G_tを使うぶん更新1回あたりの分散は大きいが、毎エピソード仕切り直しになるためActor-Criticほどの「崩れたら戻れない」脆さは今回の実行では見られなかった。この結果は、PPOがActorの更新幅を制限するclip機構を、A3Cが方策のエントロピー正則化を導入している理由——単純な1ステップActor-Criticは効率的だが不安定になりやすい——を実測を通じて裏付けるものになった。
...
| ファイル | 内容 |
|---|---|
| [01_fft_pcm_basics.py](stage14/01_fft_pcm_basics.py) | PCM(サンプリング・量子化)とFFTをスクラッチ実装し、エイリアシング・量子化誤差・周波数復元を確認 |
| [02_spectrogram_mfcc.py](stage14/02_spectrogram_mfcc.py) | STFTによるスペクトログラムと、メルフィルタバンク+DCTによるMFCCをスクラッチ実装 |
| [03_audio_classification_project.py](stage14/03_audio_classification_project.py) | ミニプロジェクト: 4種類の波形(音色)をMFCC特徴量+ロジスティック回帰で分類 |

### 図1: fft_pcm_basics.png — PCMとFFT（[01_fft_pcm_basics.py](stage14/01_fft_pcm_basics.py)）

![サンプリング周波数によるエイリアシング、量子化ビット数と量子化誤差、3周波数混合信号のFFTによる周波数復元](stage14/fft_pcm_basics.png)

**読み取れる結果**: 20Hzの信号に対しナイキスト周波数(40Hz)を下回る24Hzでサンプリングすると、サンプル点を結んだ波形が本来より低い周波数の波に見えるエイリアシングが実際に確認できた。量子化ビット数を8→4→2bitと減らすと量子化誤差(MSE)が0.00000→0.00123→0.01745と単調に増加し、階段状のノイズが視覚的にも確認できた。ノイズを加えた50Hz・120Hz・300Hzの3周波数混合信号にFFTをかけると、時間領域では目視で分離不可能な波形からでも、上位3ピークの周波数が真の周波数と完全に一致し、周波数領域への変換の威力を確認できた。

### 図2: spectrogram_mfcc.png — スペクトログラムとMFCC（[02_spectrogram_mfcc.py](stage14/02_spectrogram_mfcc.py)）

![チャープ信号のスペクトログラム、メルフィルタバンクの形状、log-melスペクトログラム、母音らしき2音のMFCC比較](stage14/spectrogram_mfcc.png)

**読み取れる結果**: 200Hz→1800Hzへ周波数が連続的に上昇するチャープ信号のスペクトログラムでは、右肩上がりの明るい帯として周波数の時間変化が明確に可視化でき、01のFFT(信号全体を1回で変換)では失われる時間情報をSTFTが保持できることを確認した。自作したメルフィルタバンクは低域ほど密に、高域ほど疎に周波数を区切っており、人間の聴覚特性を反映した設計になっていることが図からも分かる。フォルマント周波数だけを変えた「あ」らしき音(F1=800Hz,F2=1300Hz)と「い」らしき音(F1=300Hz,F2=2300Hz)のMFCC(13次元)は絶対差の合計40.62と明確に異なる値を取り、MFCCが声道の共鳴特性(音色・母音の違い)を少数の係数で捉えられることを確認できた。

### 図3: audio_classification_project.png — ミニプロジェクト: MFCCによる音色分類（[03_audio_classification_project.py](stage14/03_audio_classification_project.py)）

![正弦波/矩形波/のこぎり波/三角波の波形例と、MFCC+ロジスティック回帰による分類の混同行列](stage14/audio_classification_project.png)

**読み取れる結果**: 基本周波数(音の高さ)を150〜600Hzでランダムに変えた4種類の波形(正弦波・矩形波・のこぎり波・三角波)を、02のMFCC(13次元、時間平均)を特徴量としてロジスティック回帰で分類したところ、テスト精度0.994(180件中179件正解)を達成した。唯一の誤分類は正解が矩形波で三角波と予測されたケースで、この2つはどちらも奇数倍音のみを含み振幅の減衰の仕方(1/n vs 1/n²)だけが異なるという音響的に近い関係にあるため、低次のMFCC係数だけでは区別が難しい場合があったと考えられる。音の高さによらず倍音構成(音色)の違いを高精度に識別できたことから、MFCCが音声認識だけでなく環境音認識・楽器音分類のような音色識別タスクにも通用する汎用的な音響特徴量であることを確認できた。
