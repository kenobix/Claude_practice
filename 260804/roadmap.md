...
fastTextはword2vecが失敗する未知語(造語5語)を全てサブワード合成で埋め込めることを確認、
Janomeでの形態素解析(単語分割の曖昧性)とGiNZAでの係り受け解析(文の木構造)により
ニューラル系とは対照的な文法規則ベースの古典的NLP手法も一通り体験した
...
- GAN / DCGAN / Pix2Pix（条件付きGANによるペア画像変換）
- Diffusion Model
...
VQ-VAEでは連続潜在空間(VAE)と対比して離散コードブック(32個全てが実際に使用された)による
表現を確認し、Pix2Pixでは条件付きGAN(エッジ画像→色つき画像)で形状はスキップ結合により
正確に再現される一方、色は入力に情報がなく本質的に決定不能という限界も観察した
...
- DQN（Deep Q-Network）/ Double DQN / Dueling Network
...
**実施内容**: [work_log.md](work_log.md#stage-12-強化学習) 参照。
多腕バンディットでUCBの効率の良さ(累積後悔68.7)を実測、Q学習/SARSAはFrozenLakeで
成功率0.51程度に収束、DQNは少ないエピソード数では本質的に高分散な学習曲線をたどることを実測、
ミニプロジェクトではREINFORCEが529エピソードで解けた基準に到達した一方DQNは800エピソードでも
届かず、価値ベース法と方策ベース法の設計思想の違いを確認した(RLHFは概念整理のみで実装は割愛)。
Double DQNの過大評価バイアス緩和効果を直接測定したところ理論と逆の結果になり原因を考察、
MountainCarでの報酬形成はエネルギーベースのポテンシャル関数が位置ベースより有効に働くことを確認、
Actor-Criticは学習途中で方策が崩壊し回復しない「破局的忘却」を観測しPPO等の安全策の必要性を裏付けた

---

## Stage 13: 探索アルゴリズム（古典AI、余力があれば） ✅完了
...
**実施内容**: BFS/DFSを迷路探索でスクラッチ実装し最短性・展開ノード数のトレードオフを確認、
Mini-Max法を三目並べに全探索実装し「双方最善なら必ず引き分け」を実証、αβ法で同じ結果を
96.7%少ないノード数で得られることを確認、MCTSがシミュレーション回数を増やすほど唯一の
最適手に収束する様子を可視化した。ミニプロジェクトではαβ法AIをランダム/自分自身/複数強度の
MCTSと計310局対戦させ、負け0回(不敗)を統計的に実証した

---

## Stage 14: 音声処理 ✅完了
**狙い**: 画像・言語・強化学習・探索とは異なる「音」というモダリティに触れる
(G検定問題集で扱われるが、それまでのStageでは未着手だった領域)

- A-D変換 / PCM(パルス符号変調) / サンプリング周波数とエイリアシング / 量子化ビット数
- FFT(高速フーリエ変換) / 短時間フーリエ変換(STFT) / スペクトログラム
- メル尺度 / メルフィルタバンク / MFCC(メル周波数ケプストラム係数)

**実装**: numpy/scipyでPCM・FFT・STFT・MFCCを全てスクラッチ実装し、合成した音声信号で仕組みを確認する
**ミニプロジェクト**: 4種類の波形(音色)をMFCC特徴量+ロジスティック回帰で分類する
**実施内容**: [work_log.md](work_log.md#stage-14-音声処理) 参照。
サンプリング周波数を元信号の2倍(ナイキスト周波数)未満にするとエイリアシングが実際に生じること、
量子化ビット数を8→4→2bitと減らすと量子化誤差が単調に増加することを確認。ノイズを加えた
3周波数混合信号からFFTで真の周波数を正しく復元、チャープ信号のスペクトログラムで時間分解能を、
自作メルフィルタバンク+DCTによるMFCCで母音らしき合成音の音色差を捉えられることを確認した。
ミニプロジェクトでは正弦波/矩形波/のこぎり波/三角波(音の高さはランダム化)をMFCC特徴量で
分類しテスト精度0.994を達成、矩形波/三角波の1件の混同から倍音構成の音響的な近さを考察した
...
## 次のアクション
G検定問題集(第7弾)との照合で見つかったStage 5・6・7・9・12の未着手トピックと、
新設のStage 14(音声処理)が全て完了した。次に取り組みたいテーマがあれば教えてください。
